...
function buildSliderTemplate() {
	...
	let slideImage = (...ind) => {
		...
		let hide = () => {
			sliderImage.setAttribute("class","sliderImageHide");
			for (let image of imagesThumbnails) image.setAttribute("class", "thumbnails");
		}

		let show = () => {
			sliderImage.setAttribute("src",filteredImages[imageIndex].URL);
			sliderImage.setAttribute("title",`Album: album`);
			sliderImage.setAttribute("class","sliderImageShow");
